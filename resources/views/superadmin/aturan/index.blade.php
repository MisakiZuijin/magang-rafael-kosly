...
@section('title', 'Kelola Aturan Kos - Super Admin')
...
<div class="space-y-4" x-data="{ 
    ...
    openEditModal(aturan) {
        ...
        this.editUrl = '{{ url('/superadmin/aturan') }}/' + aturan.id;
        this.modalEdit = true;
    }
}">
    {{-- Header --}}
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Tambah, edit, dan hapus tata tertib untuk setiap kos (Super Admin)</p>
        </div>
    </div>
        ...
        <div x-show="filterKosId === 'all' || filterKosId == '{{ $aturan->kos_id }}'"
            ...
            <div class="flex justify-between items-start gap-2">
                ...
                <div class="flex items-center gap-1.5">
                    ...
                    <form action="{{ route('superadmin.aturan.destroy', $aturan->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus aturan ini?')">
                        @csrf
                        @method('DELETE')
    ...
    <x-modal show="modalTambah" title="Tambah Aturan Kos Baru">
        <form action="{{ route('superadmin.aturan.store') }}" method="POST" class="space-y-3.5">
            @csrf
