...
@section('title', 'Broadcast & Pengumuman - Super Admin')
...
<div class="space-y-4">
    ...
    <x-btn href="{{ route('superadmin.pengumuman.create') }}" size="sm" class="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-2xl shadow-sm active:scale-95 transition-all text-xs flex items-center justify-center gap-1.5">
    ...
    <div class="space-y-3">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl p-4 border border-gray-200 dark:border-gray-800 shadow-sm space-y-2.5">
            ...
            <div class="flex items-center justify-between text-[11px] text-gray-500 dark:text-gray-400 pt-1 border-t border-gray-100 dark:border-gray-800">
                <span>Dibuat oleh: <strong class="text-gray-700 dark:text-gray-300">{{ $p->dibuatOleh->nama ?? ($p->pembuat->nama ?? 'Super Admin') }}</strong></span>
            </div>
            <div class="flex items-center justify-between text-[11px] text-gray-500 dark:text-gray-400 pt-1 border-t border-gray-100 dark:border-gray-800">
