...
@section('title', 'Pendaftaran Kos & Kamar - Super Admin')
...
<div class="space-y-4" x-data="{ 
    ...
    openEditKosModal(kos) {
        ...
        this.editKosUrl = '{{ url('/superadmin/kos') }}/' + kos.id;
    ...
    openEditKamarModal(kamar) {
        ...
        this.editKamarUrl = '{{ url('/superadmin/kamar') }}/' + kamar.id;
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Kelola kos, kamar, dan penempatan anak kos (Super Admin)</p>
        </div>
    </div>
                    ...
                    <div x-show="filterTipeKamar === 'all' || filterTipeKamar === '{{ $kamar->tipe }}'"
                        ...
                        <div class="mt-2 pt-2 border-t border-emerald-200/60 dark:border-emerald-900/40 space-y-1.5">
                            ...
                            <div class="pt-1 flex justify-end">
                                <form action="{{ route('superadmin.kamar.kosongkan', $kamar->id) }}" method="POST" onsubmit="return confirm('Kosongkan Kamar {{ $kamar->kode_kamar }} dan selesaikan sewa penghuni?')">
                                    @csrf
    ...
    <x-modal show="modalKos" title="Daftarkan Kos Baru">
        <form action="{{ route('superadmin.kos.store') }}" method="POST" class="space-y-3">
            @csrf
    ...
    <x-modal show="modalKamar" title="Daftarkan Kamar Baru">
        <form action="{{ route('superadmin.kamar.store') }}" method="POST" class="space-y-3">
            @csrf
    ...
    <x-modal show="modalPenghuni" title="Daftarkan Penghuni ke Kamar">
        <form action="{{ route('superadmin.penghuni.daftar') }}" method="POST" class="space-y-3">
            @csrf
