...
@section('title', 'Broadcast & Pengumuman - Mitra Pro')
...
<div class="space-y-4" x-data="{
    ...
    {{-- Header --}}
    <x-page-header title="Broadcast Pengumuman" subtitle="Riwayat broadcast pesan dan pengumuman untuk kos Anda" backUrl="{{ route('dashboard') }}" />

    <x-btn href="{{ route('mitra.pengumuman.create') }}" size="sm" class="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-2xl shadow-sm active:scale-95 transition-all text-xs flex items-center justify-center gap-1.5">
    ...
    <x-empty-state message="Belum ada pengumuman yang dikirim. Klik + Buat Pengumuman Baru untuk memulainya." />
    @else
    <div class="space-y-3">
        ...
        $itemData = [
            ...
            'tipe' => $p->tipe,
            'channel' => $p->channel,
            'pembuat' => $p->dibuatOleh->nama ?? 'Mitra Pro',
            'target' => $targetDesc,
        ...
        <div @click="openDetail({{ json_encode($itemData) }})"
            ...
            <div class="grid grid-cols-1 gap-2 text-[11px] text-gray-500 dark:text-gray-400 pt-1 border-t border-gray-100 dark:border-gray-800">
                <span class="px-2 py-1 rounded-lg bg-emerald-50 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300 font-bold border border-emerald-200 dark:border-emerald-800 truncate">
                    {{ $targetDesc }}
                </span>
            </div>
        </div>
    ...
    <x-modal show="showModal" title="Detail Pengumuman">
                ...
                <div class="space-y-1">
                    <div class="flex items-center gap-2 flex-wrap mb-1">
                        <span class="px-2 py-0.5 text-[10px] uppercase font-bold rounded-md"
                            :class="{
                                'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300': selectedP.tipe === 'pembayaran',
                                'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300': selectedP.tipe === 'aturan',
                                'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300': selectedP.tipe === 'info'
                            }"
                            x-text="selectedP.tipe.toUpperCase()">
                        </span>
                        <span class="px-2 py-0.5 text-[10px] font-bold rounded-md bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300"
                            x-text="selectedP.channel === 'whatsapp' ? 'WhatsApp' : (selectedP.channel === 'keduanya' ? 'Web & WA' : 'Web App')">
                        </span>
                        <span class="text-xs text-gray-400 font-mono" x-text="selectedP.created_at"></span>
                    </div>
                    <h3 class="text-base font-bold text-gray-900 dark:text-white" x-text="selectedP.judul"></h3>
                </div>

                <div class="p-3 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 rounded-xl text-xs space-y-1 text-emerald-800 dark:text-emerald-300">
                    <p class="font-bold uppercase tracking-wider text-[10px]">Sasaran Pengiriman:</p>
                    <p class="font-semibold" x-text="selectedP.target"></p>
                </div>

                <div class="space-y-1">
                    <p class="text-[11px] font-bold text-gray-500 uppercase">Isi Pesan:</p>
                    <div class="p-4 bg-gray-50 dark:bg-gray-800/60 rounded-xl border border-gray-200 dark:border-gray-700 text-xs text-gray-800 dark:text-gray-200 whitespace-pre-wrap leading-relaxed" x-text="selectedP.isi"></div>
                </div>

                <div class="flex justify-end pt-2">
                    <x-btn type="button" variant="secondary" size="sm" @click="showModal = false">Tutup</x-btn>
                </div>
            </div>
        </template>
    </x-modal>
