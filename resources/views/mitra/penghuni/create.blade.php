...
@section('title', 'Daftarkan Akun Penghuni')

@section('content')
<div class="space-y-4">
    <x-page-header 
        title="Daftar Akun Penghuni" 
        subtitle="Buat akun login baru untuk calon penghuni kos Anda" 
        backUrl="{{ route('mitra.penghuni.index') }}" 
    />

    <div class="bg-white dark:bg-gray-900 rounded-2xl p-4 border border-gray-200 dark:border-gray-800 shadow-sm">
        <form action="{{ route('mitra.penghuni.store') }}" method="POST" class="space-y-3.5">
            @csrf

            <div class="p-3 bg-purple-50 dark:bg-purple-950/30 rounded-xl border border-purple-100 dark:border-purple-900/40 flex items-center gap-2.5 text-xs text-purple-700 dark:text-purple-300">
                <span class="text-base">👤</span>
                <div>
                    <p class="font-bold">Role: Penghuni Kos</p>
                    <p class="text-[11px] text-purple-600 dark:text-purple-400">Akun yang didaftarkan akan dapat login ke aplikasi untuk melihat info kamar, riwayat pembayaran, aturan kos, dan notifikasi.</p>
                </div>
            </div>

            <div>
                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Nama Lengkap Penghuni</label>
                <input type="text" name="nama" value="{{ old('nama') }}" required placeholder="Contoh: Muhammad Rafli"
            ...
            <div>
                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Email Penghuni</label>
                <input type="email" name="email" value="{{ old('email') }}" required placeholder="penghuni@contoh.com"
                    class="w-full px-3.5 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-xs text-gray-900 dark:text-white focus:ring-emerald-500 focus:outline-none">
                @error('email') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">No. HP / WhatsApp (Aktif) <span class="text-red-500">*</span></label>
            ...
            <div x-data="{ show: false }">
                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Konfirmasi Password</label>
                <div class="relative">
                    <input :type="show ? 'text' : 'password'" name="password_confirmation" required placeholder="Ketik ulang password"
                        class="w-full px-3.5 py-2.5 pr-10 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-xs text-gray-900 dark:text-white focus:ring-emerald-500 focus:outline-none">
                    <button type="button" @click="show = !show" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 focus:outline-none flex items-center justify-center w-4 h-4">
                        <svg x-show="!show" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12c.729-2.3 2.184-4.24 4.095-5.59A10.84 10.84 0 0112 5c1.884 0 3.696.486 5.27 1.341 1.91 1.35 3.365 3.29 4.095 5.59-.73 2.3-2.184 4.24-4.095 5.59A10.84 10.84 0 0112 19c-1.884 0-3.696-.486-5.27-1.341C4.82 16.31 3.365 14.37 2.036 12z" />
