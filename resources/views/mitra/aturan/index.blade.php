...
<div class="space-y-5 pb-10" x-data="{ 
    ...
    @if($kosList->isEmpty())
    <x-empty-state message="Anda belum memiliki properti kos. Silakan daftarkan kos terlebih dahulu." />
    @else
    <div class="space-y-6">
        ...
        <form action="{{ route('mitra.aturan.store') }}" method="POST" class="space-y-4">
            ...
            <div class="p-3.5 rounded-2xl border flex items-center gap-3 text-xs shadow-2xs"
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <p class="text-xs font-semibold mt-0.5 truncate" x-text="tambahKosId === 'all' ? 'Aturan ini akan otomatis diterapkan ke seluruh ({{ $kosList->count() }}) kos Anda.' : tambahKosNama"></p>
                </div>
            </div>
