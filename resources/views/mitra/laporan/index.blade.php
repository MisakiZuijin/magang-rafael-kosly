...
@section('title', 'Laporan & Keuangan - Mitra Pro')

@section('content')
<div class="space-y-4" x-data="{ tab: 'grafik', searchKos: '' }">
    {{-- Header --}}
    <x-page-header title="Laporan & Keuangan" subtitle="Grafik okupansi, rekap pendapatan kos, dan download laporan Excel" backUrl="{{ route('dashboard') }}" />

    {{-- Tombol Download Excel di bawah header --}}
    <div class="grid grid-cols-1">
        <x-btn href="{{ route('mitra.laporan.export') }}" variant="secondary" size="sm" class="!min-h-[36px] !py-1 text-xs flex items-center justify-center gap-1.5 border border-emerald-500/40 text-emerald-700 dark:text-emerald-300 bg-emerald-50/50 hover:bg-emerald-100 dark:bg-emerald-950/40 dark:hover:bg-emerald-900/60 transition-all font-bold">
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl p-4 border border-gray-200 dark:border-gray-800 shadow-sm space-y-2.5">
        ...
        <form action="{{ route('mitra.laporan.filter') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <div>
    ...
    {{-- Sub Tabs --}}
    <div class="grid grid-cols-3 gap-1.5 p-1 bg-gray-100 dark:bg-gray-800/80 rounded-xl">
    ...
    <div x-show="tab === 'grafik'" class="space-y-3" x-transition>
        {{-- Card 1: Okupansi Kamar Kos Anda --}}
        @php
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl p-4 border border-gray-200 dark:border-gray-800 shadow-sm space-y-4" x-data="{ pctTerisi: {{ $pctTerisi }}, pctKosong: {{ $pctKosong }} }">
            <h3 class="font-bold text-sm text-gray-900 dark:text-white flex items-center gap-1.5">
                ...
                <span>Persentase & Okupansi Kamar Kos Anda</span>
            </h3>
            ...
            <div class="pt-2 border-t border-gray-100 dark:border-gray-800 text-center">
                <p class="text-xs text-gray-400">Total Keseluruhan: <strong class="text-gray-700 dark:text-gray-300 font-mono">{{ $totalKamar }} Kamar</strong></p>
            </div>
        </div>

        {{-- Card 2: Grafik Pendapatan Per Bulan Khusus Mitra --}}
        @php
        $bulanList = [];
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl p-4 border border-gray-200 dark:border-gray-800 shadow-sm space-y-4">
            <div class="pb-2 border-b border-gray-100 dark:border-gray-800 space-y-1.5">
                <div>
                    ...
                    <p class="text-[10px] text-gray-400 mt-0.5">Tren pendapatan sewa kos Anda selama 6 bulan terakhir</p>
                </div>
                <div>
            ...
            <div class="space-y-3 pt-3 border-t border-gray-100 dark:border-gray-800">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Rincian Riwayat 6 Bulan Terakhir</span>
                    <span class="text-[11px] font-medium text-gray-400">Terbaru &rarr; Terlama</span>
                </div>
...
<div x-show="tab === 'pembayaran'" class="space-y-3" x-transition x-cloak>
    <div class="bg-white dark:bg-gray-900 rounded-2xl p-4 border border-gray-200 dark:border-gray-800 shadow-sm space-y-3">
        ...
        <x-empty-state message="Belum ada transaksi pembayaran terverifikasi." />
        @else
        <div class="space-y-2.5 max-h-[360px] overflow-y-auto no-scrollbar pr-0.5">
            ...
            @endphp
            <div class="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-800 flex items-center justify-between gap-2 hover:border-emerald-200 dark:hover:border-emerald-800/50 transition-all text-xs">
                <div class="min-w-0 flex-1 pr-2">
                    <p class="font-bold text-gray-900 dark:text-white truncate">{{ $penghuniNama }}</p>
                    <p class="text-[10px] text-gray-500 font-mono truncate">{{ $kosKamar }}</p>
                    <p class="text-[10px] text-gray-400 font-mono mt-0.5">Tgl: {{ $tglBayar }}</p>
                </div>
...
<div x-show="tab === 'perkos'" class="space-y-4" x-transition x-cloak>
    ...
    <div class="space-y-3">
        <div class="relative">
            ...
            <input type="text"
                x-model="searchKos"
                placeholder="Cari nama gedung kos atau alamat..."
    ...
    @php
    $totalKamarKos = $kos->kamar->count();
    $terisiKos = $kos->kamar->where('status', 'terisi')->count();
    $kosongKos = $kos->kamar->where('status', 'kosong')->count();
