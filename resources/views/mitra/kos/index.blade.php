...
@section('title', 'Kelola Kos & Kamar (Mitra Pro)')
...
<div class="space-y-4" x-data="{ 
    ...
    editKosData: { id: '', nama: '', alamat: '', link_gmaps: '', bank: '', no_rekening: '', nama_pemilik_rekening: '' },
    editKosUrl: '',
    ...
    openEditKosModal(kos) {
        ...
        this.editKosUrl = '/mitra/kos/' + kos.id;
    ...
    openEditKamarModal(kamar) {
        ...
        this.editKamarUrl = '/mitra/kos/kamar/' + (kamar.kode_kamar || kamar.id);
    ...
    {{-- Header --}}
    <x-page-header title="Kelola Kos & Kamar" subtitle="Manajemen mandiri kos, kamar, dan penghuni Anda" backUrl="{{ route('mitra.dashboard') }}">
        <div class="flex items-center gap-2">
            <span class="px-2.5 py-1 text-[10px] font-extrabold bg-gradient-to-r from-amber-500 to-yellow-400 text-gray-950 rounded-lg uppercase tracking-wider shadow-sm flex items-center gap-1">
                <span>⭐</span> PRO
            </span>
        </div>
    </x-page-header>
    ...
    {{-- Daftar Kos List --}}
    @if($kosList->isEmpty())
    <x-empty-state message="Anda belum memiliki properti kos yang didaftarkan. Klik + Kos Baru untuk memulainya." />
    @else
    @php
                ...
                <div class="flex flex-wrap items-center justify-between gap-2.5">
                    ...
                    <div x-data="{ openKosDropdown: false }" class="relative flex-shrink-0" @click.outside="openKosDropdown = false">
                        ...
                        <div x-show="openKosDropdown"
                            ...
                            class="absolute right-0 mt-1 w-36 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 py-1 z-30"
                            x-cloak>
                            ...
                            {{-- Tombol Hapus Kos --}}
                            <form action="{{ route('mitra.kos.destroy', $kos->slug ?? $kos->id) }}" method="POST" class="m-0" onsubmit="return confirm('Apakah Anda yakin ingin menghapus Kos {{ addslashes($kos->nama) }}? Seluruh kamar dan data di dalamnya juga akan terhapus.')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                    ...
                    <div x-show="matchKamar(@js($kamarMeta), @js($kosSearchText))"
                        ...
                        <div class="flex items-start justify-between gap-2 border-b border-gray-200/60 dark:border-gray-700/60 pb-2.5">
                            ...
                            <div x-data="{ openDropdown: false }" class="relative flex-shrink-0" @click.outside="openDropdown = false">
                                ...
                                <div x-show="openDropdown"
                                    ...
                                    x-cloak>
                                    <a href="{{ route('mitra.kos.kamar.show', $kamar->kode_kamar ?? $kamar->id) }}"
                                    ...
                                    <div class="border-t border-gray-100 dark:border-gray-700 my-1"></div>
                                    <form action="{{ route('mitra.kos.kamar.destroy', $kamar->kode_kamar ?? $kamar->id) }}" method="POST" class="m-0" onsubmit="return confirm('Apakah Anda yakin ingin menghapus Kamar {{ addslashes($kamar->kode_kamar) }}?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="w-full flex items-center gap-2 px-3 py-1.5 text-xs text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/50 transition-colors cursor-pointer text-left">
                        ...
                        <div class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700/80 space-y-2">
                            ...
                            <div class="p-3 rounded-xl border {{ $isPkExpired ? 'bg-red-50/70 dark:bg-red-950/40 border-red-200 dark:border-red-900/60' : 'bg-gray-50/70 dark:bg-gray-800/60 border-gray-200 dark:border-gray-700/60' }} space-y-2 shadow-2xs">
                                ...
                                <div class="flex items-center justify-between gap-2">
                                    ...
                                    <a href="{{ $waUrl }}" target="_blank"
                                        ...
                                        <span class="font-mono">{{ $pk->penghuni->no_hp }}</span>
                                    </a>
                                    @endif
                                </div>
                            ...
                            <div class="pt-2 border-t border-gray-100 dark:border-gray-800 flex items-center gap-2">
                                ...
                                <a href="{{ route('mitra.pengumuman.create', ['kamar_id' => $kamar->id]) }}" class="flex-1 min-w-0 px-2.5 py-1.5 text-[10px] font-semibold text-amber-800 dark:text-amber-200 bg-amber-100 hover:bg-amber-200 dark:bg-amber-950/80 dark:hover:bg-amber-900 rounded-xl border border-amber-300 dark:border-amber-800 inline-flex items-center justify-center gap-1.5 active:scale-95 transition-all animate-pulse text-center shadow-2xs" title="Kirim Pengumuman Jatuh Tempo ke Kamar {{ $kamar->kode_kamar }}">
                                ...
                                <form action="{{ route('mitra.kamar.kosongkan', $kamar->kode_kamar ?? $kamar->id) }}" method="POST" onsubmit="return confirm('Kosongkan Kamar {{ $kamar->kode_kamar }} dan selesaikan sewa penghuni?')" class="{{ $hasExpiredPenghuni ? 'flex-1 min-w-0' : 'w-full' }}">
                                    @csrf
    ...
    <x-modal show="modalKos" title="Daftarkan Kos Baru (Mitra Pro)">
        <form action="{{ route('mitra.kos.store') }}" method="POST" enctype="multipart/form-data" class="space-y-3">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="nama" required placeholder="Contoh: Kos Melati Eksklusif" class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-xs text-gray-900 dark:text-white">
            </div>

            <div>
                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Foto Utama Kos (Opsional)</label>
            ...
            <div>
                ...
                <textarea name="alamat" rows="2" required placeholder="Alamat jalan, nomor, kelurahan, kecamatan" class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-xs text-gray-900 dark:text-white"></textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-2">
                <div>
                    ...
                    <input type="text" name="bank" required placeholder="Contoh: BCA / Mandiri / BRI" class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-xs text-gray-900 dark:text-white">
                </div>
                <div>
    ...
    <x-modal show="modalEditKos" title="Edit Data Kos">
        <form :action="editKosUrl" method="POST" enctype="multipart/form-data" class="space-y-3">
            ...
            <div>
                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Ganti Foto Utama Kos (Opsional)</label>
                <input type="file" name="foto" accept="image/*" class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-xs text-gray-900 dark:text-white">
            ...
            <div class="grid grid-cols-2 gap-2">
                <div>
                    <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Bank Pembayaran <span class="text-red-500">*</span></label>
                    <input type="text" name="bank" x-model="editKosData.bank" required placeholder="Contoh: BCA / Mandiri / BRI" class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-xs text-gray-900 dark:text-white">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">No. Rekening <span class="text-red-500">*</span></label>
    ...
    <x-modal show="modalKamar" title="Daftarkan Kamar Baru">
        <form action="{{ route('mitra.kamar.store') }}" method="POST" enctype="multipart/form-data" class="space-y-3" x-data="{ kamarTipe: 'standar', kapasitas: 1, displayHargaBulan: '', rawHargaBulan: '', displayHargaMinggu: '', rawHargaMinggu: '', displayHargaHari: '', rawHargaHari: '' }">
            @csrf
            ...
            <div>
                ...
                <p class="text-[10px] text-gray-400 mt-0.5 italic">* Pilih beberapa file gambar (JPG, PNG, WEBP) untuk galeri foto kamar ini.</p>
            </div>
            ...
            <div x-data="{
                ...
                <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">Pilih Kos Anda <span class="text-red-500">*</span></label>
                <input type="text"
                    x-model="search"
                    @focus="open = true"
    ...
    <x-modal show="modalPenghuni" title="Daftarkan Penghuni ke Kamar">
        <form action="{{ route('mitra.penghuni.daftar') }}" method="POST" class="space-y-3.5" x-data="{ durasiSewa: 'bulanan' }">
            @csrf
            ...
            <div x-data="{
                ...
                <div class="relative">
                    <input type="text"
                        ...
                        @click.away="open = false"
                        @input="open = true; selectedId = ''"
                        placeholder="Ketik nama atau no. hp penghuni..."
                        required
                        class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-xs font-semibold text-gray-900 dark:text-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all">
                ...
                <div class="mt-1 flex items-center justify-between text-[11px]">
                    <span class="text-gray-400 dark:text-gray-500">Khusus penghuni yang Anda daftarkan</span>
                    <a href="{{ route('mitra.penghuni.create') }}" class="text-emerald-600 dark:text-emerald-400 font-bold hover:underline flex items-center gap-0.5">
                ...
                <div x-data="{
                    ...
                    <div class="relative">
                        <input type="text"
                            ...
                            @click.away="open = false"
                            @input="open = true; selectedId = ''"
                            placeholder="Ketik nama atau no. hp penghuni ke-2..."
                ...
                <div x-data="{
                    ...
                    <div class="relative">
                        <input type="text"
                            ...
                            @click.away="open = false"
                            @input="open = true; selectedId = ''"
                            placeholder="Ketik nama atau no. hp penghuni ke-3..."
                            class="w-full px-3 py-2 bg-white dark:bg-gray-800 border border-purple-200 dark:border-purple-800 rounded-xl text-xs font-semibold text-gray-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all">
