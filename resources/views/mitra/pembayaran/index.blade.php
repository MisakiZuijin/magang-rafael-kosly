...
<div class="space-y-5" x-data="{ 
            ...
            <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                <div class="space-y-1">
                    ...
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        @endphp
                        <a href="{{ $waUrlP }}" target="_blank" class="px-2 py-0.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 text-[10px] font-bold inline-flex items-center gap-1 shadow-xs" title="Chat WhatsApp ke {{ $penghuniNama }}">
                            <span>Chat WA</span>
                        </a>
                        @endif
                    </div>
    ...
    <div x-show="tab === 'terverifikasi'" class="space-y-4" x-transition x-cloak>
        ...
        <div x-show="matchVerif('{{ addslashes($searchString) }}', '{{ addslashes($kosPureNama) }}')" class="bg-white dark:bg-gray-900 rounded-2xl p-4 sm:p-5 border border-emerald-100 dark:border-emerald-900/40 shadow-sm space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-3 border-t border-gray-100 dark:border-gray-800 text-xs text-gray-500 dark:text-gray-400 font-mono">
                <div>Tgl Bayar: <strong class="text-gray-800 dark:text-gray-200">{{ $tanggalTransferFormatted }}</strong></div>
                <div>Diverifikasi: <strong class="text-emerald-600 dark:text-emerald-400">{{ $waktuVerifFormatted }}</strong></div>
            </div>
