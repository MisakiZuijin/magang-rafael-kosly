...
<div class="space-y-4" x-data="{ 
    ...
    <div class="flex items-center justify-between">
        ...
        <x-btn href="{{ request()->is('superadmin*') ? route('superadmin.pengguna.create') : route('admin.pengguna.create') }}" size="sm" class="!min-h-[36px] !py-1 text-xs">
            + Tambah User
        </x-btn>
    </div>
        ...
        @forelse($mitras as $m)
            <div x-show="matchSearch(@js($m->nama . ' ' . $m->email . ' ' . ($m->no_hp ?? '')))"
                ...
                <div class="flex gap-2 pt-2 border-t border-gray-100 dark:border-gray-800">
                    ...
                    <form action="{{ request()->is('superadmin*') ? route('superadmin.pengguna.toggle', $m->id) : route('admin.pengguna.toggle', $m->id) }}" method="POST" class="flex-1">
                        @csrf
        ...
        @forelse($penghunis as $p)
            <div x-show="matchSearch(@js($p->nama . ' ' . $p->email . ' ' . ($p->no_hp ?? '')))"
                ...
                <div class="flex gap-2 pt-2 border-t border-gray-100 dark:border-gray-800">
                    ...
                    <form action="{{ request()->is('superadmin*') ? route('superadmin.pengguna.toggle', $p->id) : route('admin.pengguna.toggle', $p->id) }}" method="POST" class="flex-1">
                        @csrf
