...
<div class="space-y-4" x-data="{ showToken: false }">
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl p-4 border border-gray-200 dark:border-gray-800 shadow-sm space-y-3">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div class="flex items-center gap-2">
                <div class="w-2.5 h-2.5 rounded-full {{ $deviceInfo['connected'] ? 'bg-emerald-500 animate-pulse' : 'bg-red-500' }}"></div>
                <h2 class="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider">Status Koneksi Device WhatsApp</h2>
            </div>
            <span class="px-2.5 py-1 text-[10px] font-bold rounded-lg text-center self-start sm:self-auto {{ $deviceInfo['connected'] ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' : 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300' }}">
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl p-4 border border-gray-200 dark:border-gray-800 shadow-sm space-y-3">
        ...
        <form action="{{ $formRoute }}" method="POST" class="space-y-3">
            ...
            <div class="pt-1 flex justify-end">
                <x-btn type="submit" variant="primary" size="sm" class="!min-h-[38px] text-xs font-bold px-5 flex items-center gap-1.5 shadow-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />
                    </svg>
                    <span>Simpan Pengaturan Fonnte</span>
                </x-btn>
            </div>
        </form>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl p-4 border border-gray-200 dark:border-gray-800 shadow-sm space-y-3">
        ...
        <form action="{{ $testRoute }}" method="POST" class="space-y-3">
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">
                    Target Penerima (Nomor HP / ID Grup WA) <span class="text-red-500">*</span>
                </label>
                <input type="text"
                    name="target"
            ...
            <div>
                ...
                <textarea name="pesan" rows="3" required placeholder="Tuliskan pesan tes pengiriman WhatsApp..."
                    class="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-xs text-gray-900 dark:text-white focus:ring-blue-500">Halo! Ini adalah pesan uji coba dari aplikasi {{ $appName }} menggunakan WhatsApp Gateway Fonnte.</textarea>
            </div>

            <div class="pt-1 flex justify-end">
                <x-btn type="submit" variant="primary" size="sm" class="!min-h-[38px] text-xs font-bold px-5 bg-blue-600 hover:bg-blue-700 flex items-center gap-1.5 shadow-sm">
    ...
    <div class="bg-gray-50 dark:bg-gray-800/40 rounded-2xl p-4 border border-gray-200 dark:border-gray-800 space-y-2 text-xs">
        <h3 class="font-bold text-gray-900 dark:text-white flex items-center gap-1.5">
            ...
            <span>Cara Registrasi & Sambungkan WhatsApp di Fonnte:</span>
        </h3>
